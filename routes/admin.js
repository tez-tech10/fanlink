...
router.get('/users', auth, adminOnly, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT u.id, u.email, u.name, u.plan, u.promo_expires_at, u.created_at, p.username FROM users u LEFT JOIN pages p ON p.user_id=u.id ORDER BY u.created_at DESC'
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Set premium for a user
