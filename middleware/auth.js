...
router.post('/signin', async (req, res) => {
  ...
  } catch (e) { console.error('signin error:', e.message); res.status(500).json({ error: e.message }); }
});

router.get('/me', auth, async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT id,email,name,plan,promo_expires_at,created_at,is_admin FROM users WHERE id=$1', [req.user.id]);
    res.json(rows[0]);
